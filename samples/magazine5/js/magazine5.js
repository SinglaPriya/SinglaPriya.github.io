...
(function($) {
	...
	var sampleName = 'magazine5',
		samplePath = 'samples/magazine5/';
	...
	function loadPage(page) {
		...
		var img = $('<img />');
		img.load(function() {
			var container = $('.magazine5 .p'+page);
			img.css({width: '100%', height: '100%'});
			img.appendTo($('.magazine5 .p'+page));
			...
			when: {
				...
				change: function(event, scale) {
					...
					if (scale==1) {
						...
						$('.magazine5').removeClass('animated').addClass('zoom-in');
				...
				zoomOut: function () {
					...
					setTimeout(function(){
						$('.magazine5').addClass('animated').removeClass('zoom-in');
					}, 0);
