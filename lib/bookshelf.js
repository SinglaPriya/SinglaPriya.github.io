...
(function ($) {
  ...
  var currentDemo,
    ...
    samples = {
      ...
      'magazine4': { 'path': 'samples/magazine4/' },
      'magazine5': { 'path': 'samples/magazine5/' },
      'magazine6': { 'path': 'samples/magazine6/' }
    },
    status = {
      'unloaded': 0,
      'loading': 1,
  ...
  bookshelf = {
    loadSample: function (demoName, callback) {
      // checking loadsample call stacj

      console.trace("loaded sample " + demoName);
    ...
    showSample: function () {
      ...
      // checking showsample call stcak
      console.trace("sample show " + currentDemo)
      ...
      if (Modernizr.csstransforms) {
        ...
        $('#book-zoom').
          ...
          removeClass('animate').
          transform('');
        //width and height becuase of book wrapper, height of thumbnail defined by (bookshelf .thumb1,.bookshelf-row .thumb1)
        console.log($('#book-zoom').width(), $('#book-zoom').height(), $('.shelf .sample[sample="' + currentDemo + '"]').height(), $('#book-zoom').offset(), $('.shelf .sample[sample="' + currentDemo + '"]').offset());
        ...
        var transitionEnd = $.cssTransitionEnd(),
          ...
          showExtras = function (e) {

            if (currentDemo == actualDemo) {
              $('.splash').addClass('show-extras');
        ...
        $('.splash').addClass('preview show-samples show-extras');
    ...
    close: function () {
      ...
      // $('.samples').css({backgroundColor: 'white'});
      ...
      if (Modernizr.csstransforms) {
        ...
        $('.splash').removeClass('show-extras');
        $('.samples .group').removeClass('greyBg');
        $('#book-zoom').css({visibility:'hidden'});
        ...
        $('.splash').removeClass('preview show-samples show-extras sample-' + smpl);
      }
    ...
    open: function (demoName) {
      ...
      // checking open call stack

      console.trace("sampled open " + demoName);
      $('.samples .group').addClass('greyBg');
      // $('.samples').css({backgroundColor: ' rgb(37, 37, 37)'});
    ...
    preload: function () {
      ...
      var test = [],
        ...
        loadNextTest = function () {
          ...
          // console.log(sample.path+" "+sample.callback+" "+sample.loading);
          ...
          if (sample.loading == status.loading) {
            ...
            console.trace("pre loading " + demoName);
            // console.log(sample)
    ...
    zoomOutButton: function (show) {
      ...
      if (!zoomOutButton) {
        zoomOutButton = $('<i />', { 'class': 'fas fa-search-minus icon zoom-out' }).
          appendTo($('.splash')).
          mouseover(function () {
  ...
  $(document).ready(function () {
    ...
    $('.share .icon').bind($.mouseEvents.over, function (e) {
      ...
      var className = $.trim($(this).
        attr('class').
        replace(/\b([a-z-]*hover|icon|fas|fab|fa-[a-z-]*)\b/g, ''));
  ...
  $(window).load(function () {
    ...
    // checking windows load

    console.trace("windows loaded")
    ...
    Hash.on('samples\/([^\/]+)(?:\/([0-9]*))?$', {
      yep: function (path, parts) {
        ...
        console.trace("in hash.on path " + path + " parts " + parts + " " + parts[1] + " " + parts[2]);
